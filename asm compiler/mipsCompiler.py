...
def splice_file_input(file_input):  # find comma placement within the current line of the file
        ...
        commands.append(tmp_array)  # add the tmp_array to the master array
...
def opcode_conversion():
        num_of_secondary_arrays = int(len(commands))  # Find the number of second dimension arrays in the master array
        for c in range(0, num_of_secondary_arrays):  # for the number of secondary arrays in commands
...
def to_binary_converter():
        num_of_secondary_arrays = int(len(commands))  # holds number 
        for c in range(0, num_of_secondary_arrays):  # for the number of secondary arrays in commands
                if commands[c][0] == 'j' or commands[c][0] == 'jal':  # If encoder is a jump instruction
                        # print('J or JAL found. Checking')
                        num_of_conversions = int(len(commands[c]))  # find the number of commands in the second dimension array
                        for sc in range(0, num_of_conversions):  # runs for the number of commands in the second dimension array
                                # print('checking')
                        ...
                        if index == 0:  # if a '$' was found and is at the beginning of the commands (to avoid conflict with hex value commands)
                                ...
                                if index == 0:
                                        ...
                                        if len(commands[c][sc]) == 4:  # if the hex value if the default length
                                                ...
                                                num_of_hex_chars = int(len(commands[c][sc]))  # get the number of string chars in the secondary command
                                                
                                                for hv in range(0, num_of_hex_chars):  # run loop for the number of single hex value chars in the command
                                                ...
                                                if commands[c][sc][4] == '(' and commands[c][sc][5] == '$' and commands[c][sc][7] == ')':  # register formatting found
                                                        ...
                                                        # commands[c][sc] = commands[c][sc].replace(commands[c][sc][4:], tmp_string_reg)
                                                        
                                                        # Hex value conversion
                                                        tmp_string_conv = ''  # Create a new string to be reference in the for loop, hold the converted command
                                                        num_of_hex_chars = int(len(commands[c][sc])) - 4  # len(tmp_string_reg)  # get the number of string chars in the secondary command
                                                        for hv in range(0, num_of_hex_chars):  # run loop for the number of single hex value chars in the command
                                                                tmp_string = commands[c][sc][hv]   # the tmp_string holds the current single hex value to be converted
                                                                tmp_string_conv += hexval[tmp_string]  # Add the newly converted hex value to the rest of the converted values
